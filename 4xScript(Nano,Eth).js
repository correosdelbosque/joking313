...
function calcBase(bits,num){
	...
	if(bits/sigma[num-1]<1){
		throwError("You need at least " + sigma[num-1] + " bits to run with your parameters, and you only set wageredBits to " + wageredBits);
		engine.stop();
	}	
	else if(bits/sigma[num-1]*bases[num-1]>maxBet){
		var thisMaxBet = maxBet;
		for(let i=0;i<num-1;i++)
			thisMaxBet /= 4;
		return Math.floor(thisMaxBet);
	}
	else{
...
function calcUnflooredBase(bits,num){
	var sigma = [1,5,21,85,341,1365,5461,21845,87381];
	var bases = [1,4,16,64,256,1024,4096,16384,65536];
	if(bits/sigma[num-1]<1){
		throwError("You need at least " + sigma[num-1] + " bits to run with your parameters, and you only set wageredBits to " + wageredBits);
